...
export default function AirLineTicket({ params }: { params: { slug: string } }) {
  ...
  if (itineraryType == '1' && searchParams.get('rd') == '') {
    dataUrl.ReturnDate = ''
  } else {
    dataUrl.ReturnDate = format(new Date(searchParams.get('rd') || Date.now()), "MM/dd/yyyy", { locale: vi })
  ...
  /* Time choose */
  const [departTime, setDepartTime] = useState(format(new Date(dataUrl.DepartureDate), 'yyyy-MM-dd', { locale: vi }))
  // const [returnTime, setReturnTime] = useState(format(new Date(dataUrl.ReturnDate), 'yyyy-MM-dd', { locale: vi }))
  /* Passengers choose */
                            ...
                            <div className=" flex flex-col px-3 py-5">
                              ...
                              <select id="baggage" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                ...
                                {returnBaggages.map((e: any, i: number) => {
                                  return (
                                    <option key={'returnBaggages'+i} value="20">Mua {e.Code} kg hành lý ({e.Price.toLocaleString()} VND)</option>
                                  )
                                })}
                              </select>
                            </div>
